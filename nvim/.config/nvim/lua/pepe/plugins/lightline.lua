-- lightline support
local M = {}

M.init = function(theme)
  if vim.g.lightline == nil then
    return
  end

  local bg0 = theme.PepeBg0.fg
  local bg1 = theme.PepeBg1.fg
  local bg2 = theme.PepeBg2.fg
  local bg4 = theme.PepeBg4.fg
  local fg1 = theme.PepeFg1.fg
  local fg4 = theme.PepeFg4.fg

  local yellow = theme.PepeYellow.fg
  local blue = theme.PepeBlue.fg
  local aqua = theme.PepeAqua.fg
  local orange = theme.PepeOrange.fg
  local red = theme.PepeRed.fg
  local green = theme.PepeGreen.fg

  local lightline_theme = {
    normal = {
      left = { { bg0, fg4, "bold" }, { fg4, bg2 } },
      middle = { { fg4, bg1 } },
      right = { { bg0, fg4 }, { fg4, bg2 } },
      error = { { bg0, red } },
      warning = { { bg0, yellow } },
    },
    insert = {
      left = { { bg0, blue, "bold" }, { fg1, bg2 } },
      middle = { { fg4, bg1 } },
      right = { { bg0, blue }, { fg1, bg2 } },
    },
    inactive = {
      left = { { bg4, bg1 } },
      middle = { { bg4, bg1 } },
      right = { { bg4, bg1 }, { bg4, bg1 } },
    },
    terminal = {
      left = { { bg0, green, "bold" }, { fg1, bg2 } },
      middle = { { fg4, bg1 } },
      right = { { bg0, green }, { fg1, bg2 } },
    },
    replace = {
      left = { { bg0, aqua, "bold" }, { fg1, bg2 } },
      middle = { { fg4, bg1 } },
      right = { { bg0, aqua }, { fg1, bg2 } },
    },
    visual = {
      left = { { bg0, orange, "bold" }, { bg0, bg4 } },
      middle = { { fg4, bg1 } },
      right = { { bg0, orange }, { bg0, bg4 } },
    },
    tabline = {
      left = { { fg4, bg2 } },
      middle = { { bg0, bg4 } },
      right = { { bg0, orange } },
      tabsel = { { bg0, fg4 } },
    },
  }

  local ll_filled = vim.fn["lightline#colorscheme#fill"](lightline_theme)
  vim.g["lightline#colorscheme#pepe#palette"] = ll_filled
  vim.schedule(function()
    vim.fn["lightline#disable"]()
    vim.fn["lightline#enable"]()
  end)
end

return M
